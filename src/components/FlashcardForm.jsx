...
export default function FlashcardForm({ onSave, editingFlashcard, setEditingFlashcard }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="card bg-white p-5 shadow-lg max-w-md mx-auto">
      ...
      {/* Category Selection */}
      <select
        className="tooltip select select-bordered w-full my-2 text-white"
        value={category}
      ...
      <button 
        type="submit" 
        className="tooltip btn btn-success w-full"
        data-tip={editingFlashcard ? "Update the existing flashcard" : "Add a new flashcard"}
      >
